...
def main_run(dataset, trainDir1, valDir, outDir, stackSize, trainBatchSize, valBatchSize, numEpochs, lr1,
    ...
    model_folder = os.path.join('./', outDir, 'flow')  # Dir for saving models and log files
        ...
        if valDir is not None:
            if (epoch+1) % 1 == 0:
                ...
                for j, (inputs, targets) in enumerate(val_loader):
                    ...
                    with torch.no_grad():
                        inputVariable = Variable(inputs.cuda(cuda1))
                        labelVariable = Variable(targets.cuda(non_blocking=True).to('cuda:1'))
                    output_label, _ = model(inputVariable)
